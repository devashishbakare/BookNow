...
describe("Test for confirm Booking Page", () => {
  ...
  test("check input feild presence", async () => {
    act(() => {
      render(
        <Router>
          <ConfirmBooking />
    ...
    const phone_numberInput = screen.getByTestId("phone_number");
    expect(phone_numberInput).toBeInTheDocument();
    await user.type(phone_numberInput, "9080706050");
    expect(phone_numberInput.value).toBe("9080706050");

    const additional_contact_informationInput = screen.getByTestId(
      "additional_contact_information"
    );
    expect(additional_contact_informationInput).toBeInTheDocument();
    await user.type(additional_contact_informationInput, "sample text");
    expect(additional_contact_informationInput.value).toBe("sample text");
  });

  test("check confirm booking button works", async () => {
    act(() => {
      render(
        <Router>
          <ConfirmBooking />
    ...
    await waitFor(() => {});
    const payOnArrivalButton = screen.getByText(/pay on arrival/i);
    await user.click(payOnArrivalButton);
    expect(payOnArrivalButton).toHaveClass("border-blue-800");
    const payOnlineButton = screen.getByText(/online payment/i);
    await user.click(payOnlineButton);
    expect(payOnlineButton).toHaveClass("border-blue-800");
  });
});
